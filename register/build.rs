use core::panic;
use std::{path::Path, vec, io::Write, collections::{HashMap, HashSet}};
use regex::Regex;
use std::{env, fs};
use walkdir::WalkDir;
...
fn write_header_out(out_name: &str) -> String {
    ...
    file_str.push_str(&format!("use {}::Simple{};\n\n", out_name, out_name.to_case(Case::Pascal)));
    file_str.push_str(&format!("pub struct Register{} {{
    pub new: fn(Simple{}) -> Simple{},
    pub load: fn(Simple{}) -> Simple{},
    pub tags: &'static quickphf::PhfMap<&'static str, &'static str>,
    pub name: &'static str,
}}\n\n", out_name.to_case(Case::Pascal), out_name.to_case(Case::Pascal), out_name.to_case(Case::Pascal), out_name.to_case(Case::Pascal), out_name.to_case(Case::Pascal)));
    file_str
}
...
fn capture_register_out(file_content: &str, file_str: &mut String, out_register: &mut Vec<String>, out_name: &str, lib_path: &str) {
...
fn write_static_outdata(file_str: &mut String, out_register: &Vec<String>, tags: &HashMap<String, Vec<(String,String)>>) {
    let mut n = 0;
    
    for i in out_register {
        file_str.push_str(&format!("pub const {}_ID: i32 = {};\n", &i.to_case(Case::UpperSnake), n));
        file_str.push_str(&format!("pub static {}_TAGS: &quickphf::PhfMap<&'static str, &'static str> = &", &i.to_case(Case::UpperSnake)));
        let mut keys = vec![];
        let mut values = vec![];
...
fn write_register_out(file_str: &mut String, out_register: &Vec<String>, out_name: &str) {
    file_str.push_str(format!("\npub static REGISTER_{}: &[Register{}] = &[\n", out_name.to_uppercase(), &out_name.to_case(Case::Pascal)).as_str());
    
    for i in out_register {
        file_str.push_str(&format!("    Register{} {{new: {}::new, load: {}::load, tags: {}_TAGS, name: {}::NAME}},\n", &out_name.to_case(Case::Pascal), i, i, &i.to_case(Case::UpperSnake), i));
    }
...
fn main() {
    ...
    for entry in WalkDir::new("..")
        ...
        if path_str.ends_with(".rs") {
            ...
            for cap in re.captures_iter(&file_content) {
                ...
                tags_values.entry(cap[2].to_string())
    ...
    for (key, value) in tags_values {
        let mut file = fs::File::create(path.join("tags_output").join(format!("{}.txt", key))).unwrap();
