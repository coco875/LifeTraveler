...
fn main() {
    ...
    for i in &block_register {
        ...
        file_block_str.push_str(&format!("pub static {}_TAGS: &quickphf::PhfMap<&'static str, &'static str> = &", i.to_uppercase()));
        let mut keys = vec![];
        let mut values = vec![];
    ...
    for i in &item_register {
        ...
        file_item_str.push_str(&format!("pub static {}_TAGS: &quickphf::PhfMap<&'static str, &'static str> = &", i.to_uppercase()));
        let mut keys = vec![];
        let mut values = vec![];
        if let Some(tags) = tags.get(i) {
            for tag in tags {
